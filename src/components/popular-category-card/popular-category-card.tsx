...
export const PopularCategoryCard:FC<PopularCategoryCardProps> = ({category}) => {
    return (
        <Link to={`/category/${category.id}`} style={{textDecoration: "none"}}>
            <div className={styles.popularCategoryCard}>
                <div className={styles.popularCategoryCard__info}>
                    <Text size="large" textType="text" textColor="black" >{category.title}</Text>
                    {category.author && <Text size="small" textType="text" textColor="dim">{category.author}</Text>}
                </div>
